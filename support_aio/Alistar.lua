...
local function OnTick()
	print(common.CountBuff(player, "AlistarE"))
	if menu.Insec:get() then
		Insec()
	end
	FlashQ()
	Engage()
	if (common.CheckBuff(player, "AlistarE") and menu.combo.waite:get()) then
		if (common.CountBuff(player, "AlistarE")) < 5 then
			...
			if (player.health / player.maxHealth) * 100 <= menu.combo.hp:get() then
				if
					(common.CheckBuffType(player, 5) or common.CheckBuffType(player, 8) or common.CheckBuffType(player, 24) or
						...
						common.CheckBuffType(player, 8) or
