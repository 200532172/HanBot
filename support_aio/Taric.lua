					...
					for i = 0, objManager.enemies_n - 1 do
						...
						if
						 ...
						 then
							...
							for z, ally in ipairs(allies) do
								if ally then
									if ally.pos:dist(player.pos) < 1200 and common.CheckBuff(ally, "taricwallybuff") then
			...
			for z, ally in ipairs(allies) do
				if ally then
					if menu.SpellsMenu.blacklist[ally.charName] and not menu.SpellsMenu.blacklist[ally.charName]:get() then
						if
							(common.CheckBuffType(player, 5) or common.CheckBuffType(player, 8) or common.CheckBuffType(player, 24) or
								common.CheckBuffType(player, 23) or
								common.CheckBuffType(player, 11) or
								common.CheckBuffType(player, 22) or
								common.CheckBuffType(player, 21))
						 then
							if ally.pos:dist(player.pos) <= spellW.range then
								player:castSpell("obj", 1, ally)
							end
...
local function OnDraw()
	if player.isOnScreen then
		...
		if menu.draws.draweally:get() then
			...
			for i, enemies in ipairs(enemy) do
				if
					enemies and not enemies.isDead and common.CheckBuff(enemies, "taricwallybuff") and
						enemies.pos:dist(player.pos) < 1200
				 then
