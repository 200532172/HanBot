...
menu.combo:boolean("qcombo", "Use Q in Combo", false)
menu.combo:boolean("wcombo", "Use W in Combo", true)

menu:menu("harass", "Harass")

menu.harass:boolean("qcombo", "Use Q in Harass", false)
...
local function AutoInterrupt(spell)
	...
	if (player.mana / player.maxMana) * 100 >= menu.SpellsMenu.mana:get() then
		if menu.SpellsMenu.blockr:get() then
			if not common.CheckBuff(player, "reapthewhirlwind") then
...
local function OnTick()
	...
	if common.CheckBuff(player, "reapthewhirlwind") then
						...
						for z, ally in ipairs(allies) do
							if ally then
								if menu.SpellsMenu.blacklist[ally.charName] and not menu.SpellsMenu.blacklist[ally.charName]:get() then
									if
										(common.CheckBuffType(ally, 5) or common.CheckBuffType(ally, 24) or common.CheckBuffType(ally, 23) or
					...
					for z, ally in ipairs(allies) do
						if ally then
							if menu.SpellsMenu.blacklist[ally.charName] and not menu.SpellsMenu.blacklist[ally.charName]:get() then
								...
								if
									(common.CheckBuffType(ally, 5) or common.CheckBuffType(ally, 24) or common.CheckBuffType(ally, 23) or
										common.CheckBuffType(plallyayer, 11) or
										common.CheckBuffType(ally, 22) or
										common.CheckBuffType(ally, 8) or
										common.CheckBuffType(ally, 21))
								 then
