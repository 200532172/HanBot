	...
	if menu.harass.wcombo:get() then
		if menu.harass.whit:get() then
			if player:spellSlot(0).state ~= 0 and player.mana > player.manaCost0 + player.manaCost1 then
				if common.IsValidTarget(target) and target then
					if (target.pos:dist(player) < spellQ.range) then
						if common.CheckBuff(target, "ZileanQEnemyBomb") then
							local pos = preds.circular.get_prediction(spellQ, target)
							if pos and pos.startPos:dist(pos.endPos) < spellQ.range then
...
local function OnTick()
	...
	if menu.misc.autoe:get() then
		for i = 0, objManager.allies_n - 1 do
			...
			if ally then
				if (common.CheckBuffType(ally, 10)) then
	...
	if menu.misc.autoq:get() then
		for i = 0, objManager.enemies_n - 1 do
			...
			if
			 ...
			 then
				if
					(common.CheckBuffType(enemies, 5) or common.CheckBuffType(enemies, 8) or common.CheckBuffType(enemies, 24) or
						common.CheckBuffType(enemies, 11) or
						common.CheckBuffType(enemies, 22) or
						common.CheckBuffType(enemies, 21))
				 then
	...
	if menu.flee.fleekey:get() then
		...
		if
			menu.flee.fleew:get() and not common.CheckBuff(player, "TimeWarp") and player:spellSlot(2).state ~= 0 and
		...
		for i = 0, objManager.allies_n - 1 do
			...
			if ally and ally.pos:dist(player.pos) <= spellR.range then
				if
				 ...
				 then
					if
						(common.CheckBuffType(ally, 23) or common.CheckBuffType(ally, 24) or common.CheckBuffType(ally, 22) or
							common.CheckBuffType(ally, 21) or
							common.CheckBuffType(ally, 18) or
							common.CheckBuffType(ally, 12))
					 then
						player:castSpell("obj", 3, ally)
					end
				end
