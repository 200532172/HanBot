...
local common = module.load("MordekaiserKornis", "common")
local spellQ = {
	range = 300
}
...
local Ghoooooooooooooooost = nil
local Dragoooooon = nil
	...
	if HasItem then
		...
		if
			(common.CheckBuffType(target, 5) or common.CheckBuffType(target, 8) or common.CheckBuffType(target, 24) or
				common.CheckBuffType(target, 10) or
...
local function AutoInterrupt(spell)
	if spell and spell.owner.type == TYPE_HERO and spell.owner.team == TEAM_ALLY and spell.owner == player then
		if spell.name:find("QAttack") then
			orb.core.set_pause_attack(player:basicAttack(0).clientAnimationTime - network.latency)
		end
	end
	if
...
function RDamage(target)
	...
	if common.CheckBuff(player, "liandrysbuff") then
		damage = (damage + damage * (0.01 * common.CountBuff(player, "liandrysbuff")))
	end
				...
				if #count_allies_in_range(player.pos, spellW.range) > 1 then
					for i = 0, objManager.allies_n - 1 do
						...
						if hero and hero.isVisible and not hero.isDead and hero.pos:dist(player.pos) <= spellW.range then
							if not common.CheckBuff(player, "mordekaiserwactive") and not common.CheckBuff(player, "mordekaiserwinactive") then
...
local function OnDraw()
	if menu.draws.drawghost:get() then
		if Ghoooooooooooooooost then
			if Ghoooooooooooooooost.isOnScreen then
		...
		if Dragoooooon then
			if Dragoooooon.isOnScreen then
...
local function GhostControl()
	if LastMovemenent < os.clock() then
		if common.CheckBuff(player, "mordekaisercotgself") then
...
local function OnTick()
	if Ghoooooooooooooooost and Ghoooooooooooooooost.isDead then
		Ghoooooooooooooooost = nil
	end
