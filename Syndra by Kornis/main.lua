...
local common = module.load("SyndraKornis", "common")
...
menu.harass:boolean("autoq", "Use Auto Q to Harass", false)
						...
						if menu.blacklist[target.charName] and not menu.blacklist[target.charName]:get() then
							if player:spellSlot(3).stacks >= menu.combo.rset.orb:get() then
								...
								if menu.combo.rset.engagemode:get() then
									local damages = RDamage(target) + QDamage(enemies) + WDamage(target) + EDamage(target)
...
local function OnDraw()
	if player.isOnScreen then
		if menu.draws.drawq:get() then
			graphics.draw_circle(player.pos, spellQ.range, 2, menu.draws.colorq:get(), 30)
		end
		if menu.draws.drawe:get() then
			graphics.draw_circle(player.pos, spellE.range, 2, menu.draws.colore:get(), 30)
		end
		if menu.draws.drawqe:get() then
			graphics.draw_circle(player.pos, spellQE.range, 2, menu.draws.colorqe:get(), 30)
		end
		if menu.draws.draww:get() then
			graphics.draw_circle(player.pos, spellW.range, 2, menu.draws.colorw:get(), 30)
		end
		if menu.draws.drawr:get() then
			graphics.draw_circle(player.pos, spellR.range, 2, menu.draws.colorr:get(), 30)
		end
	end
	...
	if (menu.draws.drawball:get()) then
		for _, objs in pairs(objSomething) do
			if objs and not objs.isDead then
				...
				graphics.draw_circle(objs.pos, 30, 2, graphics.argb(255, 255, 204, 204), 30)
