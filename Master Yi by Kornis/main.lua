...
local common = module.load("MasterYiKornis", "common")
...
menu.dodgew:header("hellow", " -- Targeted Spells -- ")
...
local function AutoInterrupt(spell) -- Thank you Dew for this <3
	...
	if
	 ...
	 then
		if not common.CheckBuff(player, "Meditate") then
			local enemyName = string.lower(spell.owner.charName)
			if dodgeWs[enemyName] then
				for i = 1, #dodgeWs[enemyName] do
					local spellCheck = dodgeWs[enemyName][i]
				...
				if
				 ...
				 then
					if menu.combo.items:get() then
						if (common.CheckBuff(player, "doublestrike")) then
							for i = 6, 11 do
...
local function OnTick()
	...
	if common.CheckBuff(player, "Meditate") then
		if orb.combat.target then
			if
				orb.combat.target and common.IsValidTarget(orb.combat.target) and
					player.pos:dist(orb.combat.target.pos) < common.GetAARange(orb.combat.target)
	...
	if not common.CheckBuff(player, "Meditate") then
		if
			menu.dodgew["Karthus" .. "R"] and menu.dodgew["Karthus" .. "R"]:get() and
				...
				(common.CheckBuffEnd(player, "karthusfallenonetarget") - game.time) * 1000 <= 300
		 then
			for i = 0, objManager.enemies_n - 1 do
				local enemies = objManager.enemies[i]
				if
		...
		if
			menu.dodgew["Zed" .. "R"] and menu.dodgew["Zed" .. "R"]:get() and common.CheckBuff(player, "zedrdeathmark") and
				(common.CheckBuffEnd(player, "zedrdeathmark") - game.time) * 1000 <= 300
		 then
			for i = 0, objManager.enemies_n - 1 do
				local enemies = objManager.enemies[i]
				if
		...
		if
			menu.dodgew["Vladimir" .. "R"] and menu.dodgew["Vladimir" .. "R"]:get() and
				...
				(common.CheckBuffEnd(player, "vladimirhemoplaguedebuff") - game.time) * 1000 <= 300
		 then
			for i = 0, objManager.enemies_n - 1 do
				local enemies = objManager.enemies[i]
				if
		...
		if
			menu.dodgew["Nautilus" .. "R"] and menu.dodgew["Nautilus" .. "R"]:get() and
				common.CheckBuff(player, "nautilusgrandlinetarget") and
				(common.CheckBuffEnd(player, "nautilusgrandlinetarget") - game.time) * 1000 <= 300
		 then
			for i = 0, objManager.enemies_n - 1 do
				local enemies = objManager.enemies[i]
				if
		...
		if
			menu.dodgew["Nocturne" .. "R"] and menu.dodgew["Nocturne" .. "R"]:get() and
				common.CheckBuff(player, "nocturneparanoiadash") and
				(common.CheckBuffEnd(player, "nocturneparanoiadash") - game.time) * 1000 <= 300
		 then
			for i = 0, objManager.enemies_n - 1 do
				local enemies = objManager.enemies[i]
				if
